...
export default async function AllReviews() {
    ...
    return (
        ...
        <button
        className="bg-purple-500 text-white px-4 py-2 rounded-full mb-4"
      ...
      {selectedCategories.length > 0 && (
        <div className="mb-4">
          <span className="text-gray-600 dark:text-gray-200">Selected Categories:</span>
          {selectedCategories.map((category) => (
            <span
              key={category}
              className="inline-block bg-purple-500 text-white px-2 py-1 rounded-full ml-2"
            >
              {category}
            </span>
          ))}
