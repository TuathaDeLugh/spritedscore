        ...
        <div className="relative" ref={dropdownRef}>
            ...
            {open ? (
                <ul className="mt-6 z-10 absolute right-0 list-none m-1 border overflow-y-auto max-h-60 md:max-h-96 dark:border-slate-700 w-ma md:w-40 rounded bg-white/70 dark:bg-slate-900/70">
                    <li key={"all"} className="text-l rounded-lg text-slate-800 dark:text-slate-300 p-1 m-2 text-center md:text-left hover:bg-purple-400 hover:text-slate-50 md:dark:hover:text-slate-200">
                        <Link onClick={() => setOpen(!open)} href={`/allreview`} className='inline-block px-1 w-full'>
                            {"All"}
                        </Link>
                    </li>
                    {ddata.map((link) => (
                        <li key={link} className="text-l rounded-lg text-slate-800 dark:text-slate-300 p-1 m-2 text-center md:text-left hover:bg-purple-400 hover:text-slate-50 md:dark:hover:text-slate-200">
