...
function AuthLinks({ navinfo }) {
    ...
    return (
      <div className="relative" ref={dropdownRef}>
        <button onClick={() => setOpen(!open)} className="border border-gray-300 dark:border-gray-500 rounded-full p-[1px]">
          {session && session.user && session.user.avatar ? (
            <Image src={session.user.avatar} height={25} width={25} alt="Img" className="rounded-full" />
          ) : (
